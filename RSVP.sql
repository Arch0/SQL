...
SELECT 
    'director_mapping', COUNT(*) AS row_count
FROM
    director_mapping 
UNION SELECT 
    'genre', COUNT(*) AS row_count
FROM
    genre 
UNION SELECT 
    'movie', COUNT(*) AS row_count
FROM
    movie 
UNION SELECT 
    'names', COUNT(*) AS row_count
FROM
    names 
UNION SELECT 
    'ratings', COUNT(*) AS row_count
FROM
    ratings 
UNION SELECT 
    'role_mapping', COUNT(*) AS row_count
FROM
    role_mapping;
...
SELECT DISTINCT
    ...
    END AS table_with_null
FROM
    movie
HAVING table_with_null IS NOT NULL;
...
SELECT count(title) AS movie_count2019
FROM movie
...
(
		SELECT count(title) as movie_count,genre
...
WITH m as
(
		SELECT count(genre) AS genre_count, movie_id ,title
		FROM genre
        INNER JOIN movie
        ON genre.movie_id = movie.id
		GROUP BY movie_id
)
SELECT count(title) as 'movies with 1 genre'
FROM m 
WHERE genre_count = 1;
...
WHERE
    country = 'usa' AND total_votes > 1000
...
-- Type your code below:
SELECT 
    distinct title, avg_rating, genre
FROM
    movie
        INNER JOIN
...
SELECT 
    sum(total_votes) AS number_of_votes, country
FROM
    movie AS m
        INNER JOIN
...
(
		SELECT 
			name AS director_name, COUNT(m.id) AS movie_count,
		...
		FROM
			...
			ratings r ON r.movie_id = m.id
				JOIN
			director_mapping d ON d.movie_id = r.movie_id
				JOIN
...
SELECT 
    ...
    RANK() OVER(ORDER BY avg_rating DESC, COUNT(title) DESC)  actor_rank
FROM
    movie
        INNER JOIN
    ratings ON movie.id = ratings.movie_id
...
SELECT 
    ...
    RANK()OVER(ORDER BY SUM(avg_rating*total_votes) / sum(total_votes) DESC,total_votes DESC) actress_rank
FROM
    movie
        INNER JOIN
    ratings ON movie.id = ratings.movie_id
...
WHERE
    avg_rating>8 and genre="drama" and category="actress"
GROUP BY name
limit 3;
...
SELECT
   ...
   t.minrating,
   t.maxrating,
   t.total_duration 	-- ,t.director_rank
FROM
   director AS t 
   INNER JOIN
      avginterd AS a 
